...
**Findings:** Over the four-year study period, we analysed [PLACEHOLDER: N] city-days comprising [PLACEHOLDER: N] respiratory hospitalisations across 27 capitals. The overall Average Treatment Effect of PM2.5 exceedance on daily respiratory hospitalisations was [PLACEHOLDER: ATE estimate with 95% CI], confirmed by Double Machine Learning ([PLACEHOLDER: DML ATE with 95% CI]). The omnibus heterogeneity test was statistically significant (p [PLACEHOLDER]), revealing [PLACEHOLDER]-fold variation in CATEs across subpopulations. Elderly adults (aged 60 years and older) showed [PLACEHOLDER]-fold higher effects than the population average, and capitals with the highest fleet density (top tercile) exhibited CATEs [PLACEHOLDER]-fold greater than those with the lowest. SHAP analysis identified [PLACEHOLDER: top 3 modifiers] as the primary drivers of heterogeneity. Under a scenario of full WHO guideline compliance, an estimated [PLACEHOLDER] hospitalisations per year would be prevented, with [PLACEHOLDER]% of the preventable burden concentrated in the highest-vulnerability tercile.
...
**Funding:** [PLACEHOLDER: Funding sources]
...
Over the four-year study period, the analytical sample comprised [PLACEHOLDER: N] city-days with complete data on air quality, weather, and hospitalisations across 27 capitals. A total of [PLACEHOLDER: N total admissions] respiratory hospitalisations (ICD-10 J00--J99) were recorded, corresponding to a mean of [PLACEHOLDER] admissions per city-day (interquartile range [PLACEHOLDER]). The mean daily PM2.5 concentration across all city-days was [PLACEHOLDER] micrograms per cubic metre (standard deviation [PLACEHOLDER]; range [PLACEHOLDER]). The treatment prevalence (PM2.5 > 15 micrograms per cubic metre) was [PLACEHOLDER]%, with marked geographic variation: [PLACEHOLDER: highest prevalence cities] exceeded the threshold on more than [PLACEHOLDER]% of days, while [PLACEHOLDER: lowest prevalence cities] exceeded it on fewer than [PLACEHOLDER]% of days.

The study population of approximately 53.2 million inhabitants included [PLACEHOLDER]% aged 0--14 years, [PLACEHOLDER]% aged 15--59, and [PLACEHOLDER]% aged 60 or older (IBGE 2022). Fleet density ranged from [PLACEHOLDER] vehicles per 1,000 inhabitants in [PLACEHOLDER: lowest city] to [PLACEHOLDER] in [PLACEHOLDER: highest city], with a national capital mean of [PLACEHOLDER].

### Average Treatment Effect

The Honest Causal Forest estimated an overall ATE of PM2.5 exceedance on daily respiratory hospitalisations of [PLACEHOLDER: ATE point estimate] additional admissions per city-day (95% CI: [PLACEHOLDER]), corresponding to an incidence rate ratio of [PLACEHOLDER] (95% CI: [PLACEHOLDER]). The DML estimate was [PLACEHOLDER] (95% CI: [PLACEHOLDER]), consistent with the forest-based ATE (p for difference = [PLACEHOLDER]). The propensity score distribution showed adequate overlap between treated and untreated city-days, with fewer than [PLACEHOLDER]% of observations requiring trimming.

### Treatment Effect Heterogeneity

The omnibus calibration test confirmed statistically significant treatment effect heterogeneity (differential forest prediction coefficient = [PLACEHOLDER], p = [PLACEHOLDER]). Estimated CATEs ranged from [PLACEHOLDER: minimum CATE with CI] to [PLACEHOLDER: maximum CATE with CI] across individual city-days, representing a [PLACEHOLDER]-fold variation.

**Age group.** The GATE for elderly adults (60+ years) was [PLACEHOLDER] (95% CI: [PLACEHOLDER]), representing [PLACEHOLDER] times the population-average ATE. Children (0--14 years) showed a GATE of [PLACEHOLDER] (95% CI: [PLACEHOLDER]), [PLACEHOLDER] times the average. Working-age adults (15--59 years) had the lowest GATE at [PLACEHOLDER] (95% CI: [PLACEHOLDER]).

**Sex.** Males and females showed GATEs of [PLACEHOLDER] and [PLACEHOLDER], respectively, with [PLACEHOLDER] showing a statistically significantly higher effect (p = [PLACEHOLDER]).

**Fleet density.** Capitals in the highest fleet density tercile (>[PLACEHOLDER] vehicles per 1,000 inhabitants) exhibited a GATE of [PLACEHOLDER] (95% CI: [PLACEHOLDER]), [PLACEHOLDER]-fold higher than the lowest tercile ([PLACEHOLDER]; 95% CI: [PLACEHOLDER]).

**Diurnal temperature range.** City-days in the highest DTR tercile (>[PLACEHOLDER] degrees C) showed a GATE of [PLACEHOLDER] (95% CI: [PLACEHOLDER]), suggesting that thermal stress amplifies the respiratory impact of PM2.5 exceedance by [PLACEHOLDER]%.

**Region.** The North showed the highest regional GATE ([PLACEHOLDER]; 95% CI: [PLACEHOLDER]), followed by the Central-West ([PLACEHOLDER]) and Northeast ([PLACEHOLDER]). The Southeast ([PLACEHOLDER]) and South ([PLACEHOLDER]) showed the lowest regional effects.

### CLAN Analysis

The CLAN analysis sorted city-days into quintiles of estimated CATE. The most-affected quintile (highest CATEs) was characterised by a mean age of [PLACEHOLDER] years, [PLACEHOLDER]% elderly population, fleet density of [PLACEHOLDER] vehicles per 1,000, and DTR of [PLACEHOLDER] degrees C. In contrast, the least-affected quintile had a mean age of [PLACEHOLDER] years, [PLACEHOLDER]% elderly, fleet density of [PLACEHOLDER], and DTR of [PLACEHOLDER] degrees C. The Best Linear Projection confirmed that age (beta = [PLACEHOLDER], p < 0.001), fleet per capita (beta = [PLACEHOLDER], p = [PLACEHOLDER]), and DTR (beta = [PLACEHOLDER], p = [PLACEHOLDER]) were the strongest linear predictors of CATE variation.

### SHAP Heterogeneity Decomposition

TreeSHAP analysis identified [PLACEHOLDER: top modifier, e.g., age group] as the most important driver of treatment effect heterogeneity (mean absolute SHAP value = [PLACEHOLDER]), followed by [PLACEHOLDER: second modifier] ([PLACEHOLDER]) and [PLACEHOLDER: third modifier] ([PLACEHOLDER]). The SHAP interaction analysis revealed a significant synergistic effect between DTR and PM2.5 treatment status: on high-DTR days, the marginal impact of PM2.5 exceedance on respiratory admissions was [PLACEHOLDER]% greater than on low-DTR days, consistent with hypothesis H4.

For the five capitals with the highest average CATEs ([PLACEHOLDER: city names]), SHAP force plots showed that [PLACEHOLDER: dominant SHAP drivers for these cities, e.g., high fleet density and large elderly population] consistently pushed CATEs above the national average. The five capitals with the lowest CATEs ([PLACEHOLDER: city names]) were characterised by [PLACEHOLDER: e.g., low fleet density, younger demographic profile, and moderate DTR].

### Policy Counterfactuals

Under a scenario of full WHO guideline compliance (PM2.5 never exceeding 15 micrograms per cubic metre), an estimated [PLACEHOLDER] respiratory hospitalisations per year would be prevented across all 27 capitals (95% CI: [PLACEHOLDER]). Of this preventable burden, [PLACEHOLDER]% was concentrated in the highest-vulnerability CATE tercile, [PLACEHOLDER]% in the middle tercile, and [PLACEHOLDER]% in the lowest tercile. The estimated annual cost savings were R$ [PLACEHOLDER] million (USD [PLACEHOLDER] million), with [PLACEHOLDER]% attributable to the most vulnerable subpopulations.

The dose-response policy curve showed that progressively stricter thresholds yielded incremental benefits: the CONAMA intermediate target (25 micrograms per cubic metre) would prevent [PLACEHOLDER] hospitalisations per year, the final standard (35 micrograms per cubic metre) would prevent [PLACEHOLDER], and the pre-2024 standard (50 micrograms per cubic metre) would prevent only [PLACEHOLDER]. The marginal benefit of moving from CONAMA to WHO compliance was [PLACEHOLDER] additional prevented hospitalisations per year.

### Sensitivity and Robustness

The placebo treatment (PM2.5 at t+7) yielded an ATE of [PLACEHOLDER] (95% CI including zero; p = [PLACEHOLDER]), supporting the validity of our causal identification strategy. ATEs estimated under alternative thresholds (25, 35, 50 micrograms per cubic metre) were [PLACEHOLDER], [PLACEHOLDER], and [PLACEHOLDER], respectively, showing the expected monotonic increase with threshold severity. The continuous treatment analysis (generalised causal forest) estimated a marginal effect of [PLACEHOLDER] additional admissions per 1 micrograms per cubic metre increase in PM2.5 (95% CI: [PLACEHOLDER]), with qualitatively similar heterogeneity patterns across age and fleet density.

Rosenbaum bounds indicated that the ATE remained significant at the 5% level for Gamma values up to [PLACEHOLDER], meaning that an unmeasured confounder would need to change the odds of PM2.5 exceedance by a factor of [PLACEHOLDER] to nullify the result. The leave-one-city-out jackknife produced ATEs ranging from [PLACEHOLDER] to [PLACEHOLDER], with no single capital's exclusion altering the significance or direction of the main finding. Alternative confounder specifications (minimal, extended, extended with co-pollutants) yielded ATEs of [PLACEHOLDER], [PLACEHOLDER], and [PLACEHOLDER], all within [PLACEHOLDER]% of the primary estimate.

The DLNM baseline estimated a pooled relative risk of [PLACEHOLDER] (95% CI: [PLACEHOLDER]) per 10 micrograms per cubic metre increase in PM2.5 at lag 0--1, broadly consistent with the causal forest ATE when converted to the same scale. City-specific DLNM estimates were correlated with city-level GATEs (Spearman rho = [PLACEHOLDER], p = [PLACEHOLDER]), providing mutual validation while demonstrating that the causal forest additionally reveals within-city heterogeneity that DLNM cannot capture.
...
The overall causal effect of PM2.5 exceeding the WHO guideline on respiratory hospitalisations, estimated at [PLACEHOLDER] additional admissions per city-day, aligns with the international literature on short-term pollution-health associations but is now established under explicit causal identification. The magnitude is consistent with the pooled relative risks reported in the ESCALA multi-city study for Latin America^9^ and with recent estimates from Chinese and European multi-city analyses,^22,23^ suggesting that the causal relationship is robust across geographic and socioeconomic contexts.

The critical finding, however, is the [PLACEHOLDER]-fold variation in treatment effects across subpopulations. This heterogeneity is not an artefact of model complexity: it was confirmed by the omnibus calibration test, is robust to seven pre-specified sensitivity analyses, and is consistent with biological mechanisms. Elderly adults bear the greatest burden, with effects [PLACEHOLDER] times the population average, consistent with age-related decline in respiratory defence mechanisms, higher prevalence of chronic respiratory disease, and reduced physiological reserve.^24^ The amplification of effects in high-fleet-density capitals points to a chronic exposure pathway: populations in vehicle-dense cities face elevated baseline pollutant concentrations that sensitise respiratory epithelium, rendering acute exceedance events more harmful.^25^ The synergistic interaction between DTR and PM2.5 -- high thermal stress amplifying pollution's respiratory impact -- is a novel finding with direct relevance to climate change adaptation, as both extreme temperature variability and pollution episodes are projected to increase in tropical cities.^26^

### Comparison with Existing Literature

Brazilian studies using GAM and case-crossover designs in Sao Paulo, Curitiba, and Rio de Janeiro have reported percentage increases in respiratory admissions per 10 micrograms per cubic metre PM2.5 ranging from 2.5% to 6.8%.^5--8^ Our ATE, when converted to the same scale, falls within this range ([PLACEHOLDER]%), providing reassurance that the causal machine learning framework produces estimates consistent with established epidemiological evidence while adding three dimensions of value: explicit causal identification, nationwide geographic coverage, and heterogeneous effect estimation.
...
Second, the dose-response policy curve -- showing that moving from CONAMA's current intermediate target (25 micrograms per cubic metre) to the WHO guideline (15 micrograms per cubic metre) would prevent an additional [PLACEHOLDER] hospitalisations annually, concentrated in the most vulnerable populations -- provides quantitative evidence for the ongoing revision of CONAMA Resolution 506/2024, which The Lancet Planetary Health has criticised as insufficiently protective.^31^

Third, the cost analysis demonstrates that the preventable burden of PM2.5 exceedance is not distributed equally: [PLACEHOLDER]% of avoidable hospital costs fall on the highest-vulnerability tercile. This finding reframes air quality regulation as an environmental justice issue and supports the constitutional principle that the Unified Health System must prioritise equity in health protection.

### Conclusion

Air pollution does not affect everyone equally. By applying causal machine learning to the largest multi-city environmental health dataset in Brazil, we have shown that the respiratory burden of PM2.5 exceedance varies by up to [PLACEHOLDER]-fold across subpopulations, with elderly adults in high-fleet-density cities bearing the greatest harm. The cost of treating everyone equally, under uniform air quality thresholds, is that the most vulnerable pay the highest price. Brazil's new National Air Quality Policy and MonitoAr system offer a once-in-a-decade window to embed heterogeneity-aware evidence into regulatory practice. The data, methods, and evidence presented here are designed to ensure that window is not missed.
...
**Figure 1.** Geospatial map of estimated Conditional Average Treatment Effects (CATEs) of PM2.5 exceedance on respiratory hospitalisations across 27 Brazilian state capitals. Dot colour represents CATE magnitude (blue = low, red = high); dot size represents confidence interval width. The map reveals geographic clustering of vulnerability, with [PLACEHOLDER: pattern description].
